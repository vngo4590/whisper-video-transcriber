...
class App:
    ...
    def _on_generate_clips_requested(self, path, model_name, max_clips, api_key, claude_model):
        self._clips.reset()
        self._notebook.select(1)   # switch to Clips tab immediately
        self._clips_controller.run(path, model_name, max_clips, api_key, claude_model)
